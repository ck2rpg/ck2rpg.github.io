...
function writeCultures() {
  ...
  const lines = [];
  lines.push('# generated_cultures.txt');
...
(function addCulturesAllInOne(){
  ...
  if(!document.getElementById('cult-roll')){
    ...
    card.innerHTML = `
      ...
      <label class="small mono">provinces / culture</label>
      <input id="cult-min" type="number" min="1" value="50" style="width:64px">
      <span class="small">to</span>
      <input id="cult-max" type="number" min="1" value="500" style="width:64px">
  ...
  // --- add "Cultures" to map-mode list (once) ---
  const levelSeg = document.getElementById('levelSeg');
  if(levelSeg && !levelSeg.querySelector('button[data-level="culture"]')){
    const btn = document.createElement('button');
    btn.className = 'btn';
    btn.dataset.level = 'culture';
    btn.textContent = 'Cultures';
...
// =======================
//
// Map-driven: uses province seeds, terrain, "latitude", coast-ness.
// Call with a culture object from worldCultures:
//
//   const picks = pickTraditionsForCulture(worldCultures[0]);
//   console.log(picks); // ["tradition_desert_nomads", "tradition_pastoralists", "tradition_seafaring"]
//
// You can also attach the result back onto each culture:
//   worldCultures.forEach(c => { c.traditions = pickTraditionsForCulture(c); });
...
function pickTraditionsForCulture(culture) {
  ...
  for (let i = 0; i < scored.length && picked.length < 3; i++) {
    // Always allow at least *some* picks, even if scores are small.
    picked.push(scored[i].id);
  }

  // Fallback: if we somehow got < 3 (e.g. no traditions array), pad with random unique ones.
...
function analyzeCultureTerrain(culture) {
  ...
  for (const p of provIds) {
    ...
    // Terrain for this province: from seed cache or terrain map
    let t = seed.terrain;
  ...
  // Normalize "latitude" (0 = far north/top, 1 = far south/bottom)
  const latNorm = H > 1 ? 1 - (avgY / (H - 1)) : 0.5;
  // Treat 0.5 as equator; distance from mid gives "polar-ness"
  ...
  const tropicish = 1 - latFromEquator; // nearer "equator line"
  const coldish   = latFromEquator;     // nearer poles
...
function scoreTraditionForFeatures(id, f) {
  // tiny baseline so "unmapped" traditions still compete a bit
  let s = 0.05;
  ...
  switch (id) {
    // --- Strongly geography-tied: deserts / drylands ---
    case "tradition_desert_nomads":
    case "tradition_dryland_dwellers":
    case "tradition_saharan_nomads":
    case "tradition_warriors_of_the_dry":
      s += arid * 3 + rugged * 0.5;
      break;

    case "tradition_pastoralists":
      s += (steppeLike * 2) + arid * 1.2 + coastShare * -0.2;
      break;

    // --- Mountains / highlands / cold ---
    case "tradition_highland_warriors":
    case "tradition_hill_dwellers":
    case "tradition_mountaineers":
    case "tradition_mountaineer_ruralism":
      s += rugged * 2 + highMtn * 1.2;
      break;

    case "tradition_himalayan_settlers":
    case "tradition_horn_mountain_skirmishing":
    case "tradition_mountain_homes":
    case "tradition_mountain_herding":
      s += highMtn * 2.5 + rugged * 1.0;
      break;

    case "tradition_upland_skirmishing":
      s += (hills + mountains) * 2.0;
      break;

    case "tradition_winter_warriors":
      s += coldish * 2.5 + (taiga + mountains) * 1.2;
      break;

    // --- Jungle / forest / wetlands ---
    case "tradition_jungle_dwellers":
    case "tradition_jungle_warriors":
      s += jungleHeavy * 3 + tropicish * 1.5;
      break;

    case "tradition_forest_fighters":
    case "tradition_forest_folk":
    case "tradition_forest_wardens":
      s += forestHeavy * 2.5 + wooded * 0.5;
      break;

    case "tradition_wetlanders":
      s += wet * 3 + (plains + farmlands) * 0.5;
      break;

    case "tradition_sacred_groves":
      s += wooded * 2 + tropicish * 0.5;
      break;

    case "tradition_sacred_hunts":
      s += (wooded + jungleHeavy) * 2 + wet * 0.5;
      break;

    // --- Coasts / seas / trade ---
    case "tradition_fishermen":
      s += coastal * 3 + arid * -0.3;
      break;

    case "tradition_seafaring":
      s += coastal * 3 + f.total * 0.0005;
      break;

    case "tradition_maritime_mercantilism":
      s += coastal * 2.5 + fertile * 0.7;
      break;

    case "tradition_practiced_pirates":
      s += coastal * 3.2 + rugged * 0.5;
      break;

    case "tradition_caravaneers":
      s += (arid + steppeLike) * 1.5 + coastal * 0.7;
      break;

    // --- Agriculture / wetlands / polders ---
    case "tradition_agrarian":
      s += fertile * 3 + wet * 0.5;
      break;

    case "tradition_polders":
      s += (wet + floodpl) * 2.5 + coastal * 1.5 + plains * 0.8;
      break;

    case "tradition_medicinal_plants":
      s += (wooded + jungleHeavy) * 2 + tropicish * 1.0;
      break;

    // --- Mining / metallurgy (mountainous) ---
    case "tradition_ancient_miners":
      s += highMtn * 3 + rugged * 0.7;
      break;

    case "tradition_metal_craftsmanship":
    case "tradition_sorcerous_metallurgy":
      s += highMtn * 2.5 + forestHeavy * 0.5;
      break;

    // --- Horse / open terrain warfare ---
    case "tradition_horse_breeder":
      s += (steppeLike + arid) * 2.2;
      break;

    case "tradition_hussar":
    case "tradition_hit_and_run":
    case "tradition_warrior_culture":
      s += steppeLike * 2 + plains * 1.0 + rugged * -0.3;
      break;

    // --- Bush / hunting / animals tied to landscape ---
    case "tradition_bush_hunting":
    case "tradition_hunters":
      s += (wooded + jungleHeavy + wetlands) * 2;
      break;

    case "tradition_lords_of_the_elephant":
      s += (jungleHeavy + wetlands + plains) * 1.5 + tropicish * 1.0;
      break;

    // --- Cold forests & taiga-ish stuff ---
    case "tradition_taiga_spirit": // if you later add one; placeholder
      s += (taiga + forest) * 2.5 + coldish * 1.5;
      break;

    // --- City / fortification hints (hills/plains/farmland mix) ---
    case "tradition_castle_keepers":
      s += (hills + mountains) * 1.7 + plains * 0.8;
      break;

    case "tradition_city_keepers":
      s += fertile * 1.8 + coastal * 0.6;
      break;

    // --- Sacred mountains ---
    case "tradition_sacred_mountains":
      s += highMtn * 3;
      break;

    // --- Misc loosely tied flavour (slight nudges) ---
    case "tradition_ep2_avid_falconers":
      s += (hills + plains + steppeLike) * 1.2;
      break;

    case "tradition_mountain_herding":
      s += highMtn * 2.3 + steppeLike * 1.0;
      break;

    default:
      // Everything else: keep baseline, maybe nudge by realm size so larger cultures
      // tend to get more "institutional" traditions.
      s += f.total * 0.0002;
      break;
  }
...
function buildCultureArray() {
  ...
  // --- Create array of empty culture objects ---
  const arr = new Array(cultureCount);
  for (let cid = 0; cid < cultureCount; cid++) {
    const rgb = rgbFromInt(culturePalette[cid]);
    arr[cid] = {
      id: `culture_${nameFromRGB(rgb)}`,   // unique stable name
      color: { R: rgb[0], G: rgb[1], B: rgb[2] },
      provinces: [],                       // push province IDs later
      group: `culture_group_${cid}`,       // placeholder
    };
  }
...
function rollCultures(){
  ...
  // We now require counties, because cultures are county-based
  ...
  // --- 2) Counties ➜ Cultures (county-based) ---
  ...
  if (constraint.name === 'global') {
    ...
    for (const countyList of bins) {
      ...
      // compact local ids to global ids
      const localToGlobal = new Map();
      for (const c of countyList) {
        const lc = tmpMap[c];
        if (lc < 0) continue;
        if (!localToGlobal.has(lc)) localToGlobal.set(lc, nextC++);
        countyToCulture[c] = localToGlobal.get(lc);
      }
    }
  }

  const cultureCountTmp = nextC;

  // --- 3) Push county cultures down to provinces ---
  ...
  // --- 4) Cultures ➜ Heritages (same idea as before) ---

  let cultureToHeritageMap = null;
  let localHeritageCount   = 0;

  if (cultureCountTmp > 0) {
    // Build culture-level adjacency by lifting province adjacency to cultures
    const cultureLift = liftAdjacency(pAdj, newProvToCulture, cultureCountTmp);

    // You can later expose [hMin,hMax] via UI
    const HERITAGE_MIN = 3;
    const HERITAGE_MAX = 12;

    const eligCulture = (_) => true; // all cultures eligible
    const { map: hMap, count: hCount } =
      groupGraph(cultureCountTmp, cultureLift.adj, eligCulture, [HERITAGE_MIN, HERITAGE_MAX]);

    cultureToHeritageMap = hMap;
    localHeritageCount   = hCount;
  } else {
    cultureToHeritageMap = new Int32Array(0);
    localHeritageCount   = 0;
  }

  cultureToHeritage = cultureToHeritageMap;
  heritageCount     = localHeritageCount;
  heritagePalette   = makeUniquePalette(heritageCount);

  // --- 5) Attach culture + heritage info to each seed (province) ---
  ...
  if (typeof setStatus === 'function') {
    ...
    setStatus(
      `Cultures & Heritages rolled (${labelStr}) in ${(performance.now() - t0 | 0)} ms • ` +
      `Cultures=${cultureCount} Heritages=${heritageCount}`
    );
  }

  // --- 7) Build culture groups + annotate them with heritage too ---
  ...
  // (Optionally, you can build a worldHeritages array here by aggregating cultures
  //  with the same heritageIndex if you want explicit heritage objects.)
}
  ...
  if(!window.__culturePatchApplied){
    ...
    renderLevel = function(level){
      if(level !== 'culture') return _renderLevelOrig(level);

      // If Voronoi hasn't run yet, fall back
      if(typeof label === 'undefined' || !label){
        return _renderLevelOrig(currentLevel === 'culture' ? 'landProvinces' : currentLevel);
      }
      ...
      for(let k=0;k<label.length;k++){
        ...
        const gid = (provToCulture && provToCulture[p] != null) ? provToCulture[p] : -1;
        const c = (gid>=0 && culturePalette[gid]!=null) ? culturePalette[gid] : (seeds[p]?.color ?? 0x666666);
      ...
      if(showEdges.checked){
        function gidAt(ix){
          ...
          return (provToCulture && provToCulture[p] != null) ? provToCulture[p] : -1;
        }
        for(let y=1;y<H-1;y++){
          for(let x=1;x<W-1;x++){
            const k=y*W+x;
  ...
  switch (family) {
    ...
    case "tropical_maritime":
      ...
      candidates.clothing = [
        ...
        "papuan_clothing_gfx",
        "indian_clothing_gfx",
        "sami_clothing_gfx" // odd but can give some fringe looks
      ];
      candidates.unit = [
        "southeast_asian_unit_gfx",
        "chinese_unit_gfx",
...
function assignVisualsFromGeo(culture) {
  ...
  const chosenEth      = chooseFrom(cands.ethnicity, key, "ethnicity");
