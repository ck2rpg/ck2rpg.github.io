  ...
  function rebuildEffectiveMask(){
    ...
    if(treatAsSea){
      ...
      // effective mask equals original
      effMask.set(landMask);
    }

    // components on effective mask (used for seeding/coverage)
    ({compId:effCompId, compSize:effCompSize, count:effCompCount} = labelComponents(effMask,1));

    // draw mask preview (effective)
    const md = mctx.createImageData(W,H); const d=md.data;
    for(let i=0;i<effMask.length;i++){
      const v = effMask[i] ? 255 : 0;
      const k=i*4; d[k]=v; d[k+1]=v; d[k+2]=v; d[k+3]=255;
    }
    mctx.putImageData(md,0,0);

    // Rerender
    label=null; resetHierarchy(); renderPreview();
  }
  ...
  // ===== Seeding =====
  function isEffLandAt(k){ return effMask && effMask[k]===1; }

  autoSeedsBtn.addEventListener('click',()=>{
    if(!effMask){alert('Load a heightmap first.');return}
    const landTarget = +landSeedsIn.value|0; const seaTarget = +seaSeedsIn.value|0;
    const cs = clamp(+cellSizeIn.value|0,4,512);
    seeds = [];

    function sampleMask(target,isLand){
        ...
        for(let gx=0; gx<cols; gx++){
          ...
          for(let y=y0; y<Math.min(y0+cs,H); y+=Math.max(1,cs>>2)){
            for(let x=x0; x<Math.min(x0+cs,W); x+=Math.max(1,cs>>2)){
              const k=idx(x,y);
              if(!!isEffLandAt(k) === isLand){ any=true; break; }
            }
            if(any) break;
          }
      ...
      for(let i=0;i<take;i++){
        ...
        do{
          ...
          tries++;
        } while(tries<60 && (!!isEffLandAt(idx(sx,sy)) !== isLand));
        if(!!isEffLandAt(idx(sx,sy)) === isLand){
          seeds.push({x:sx,y:sy,isLand,id:idBase,color:0}); idBase++;
        }
      }
    }
    ...
    // Stats on ignored islands (based on original land mask)
    const thr = Math.max(0,(+minIslandPxIn.value|0));
    let tinyCount=0, tinyPixels=0;
    for(let c=0;c<landCompCount;c++){
      if(landCompSize[c] < thr){ tinyCount++; tinyPixels += landCompSize[c]; }
    }
    const modeMsg = tinyAsSea.checked ? 'Reclassified as sea' : 'Will be merged';
    setStatus(`Placed ${seeds.filter(s=>s.isLand).length} land + ${seeds.filter(s=>!s.isLand).length} sea seeds. Tiny islands: ${tinyCount} comps / ${tinyPixels} px (${modeMsg}).`);
  });
  ...
  function ensureSeedsCoverComponents(){
    ...
    const seaC  = labelComponents(effMask,0);
    ...
    for(const s of seeds){
      const k = s.y*W + s.x;
      if(s.isLand){ const id=land.compId[k]; if(id>=0) hasLand[id]=1; }
      else{ const id=seaC.compId[k]; if(id>=0) hasSea[id]=1; }
    }

    for(let id=0; id<land.count; id++){
      if(!hasLand[id]){ const k=findFirstIndexInComp(effMask, land.compId, id, 1); if(k>=0) seeds.push({x:k%W,y:(k/W)|0,isLand:true,id:seeds.length,color:0}); }
    }
    for(let id=0; id<seaC.count; id++){
      if(!hasSea[id]){ const k=findFirstIndexInComp(effMask, seaC.compId, id, 0); if(k>=0) seeds.push({x:k%W,y:(k/W)|0,isLand:false,id:seeds.length,color:0}); }
    }

    function findFirstIndexInComp(maskArr, compArr, compWanted, wantVal){
      for(let k=0;k<compArr.length;k++) if(maskArr[k]===wantVal && compArr[k]===compWanted) return k;
      return -1;
    }
  }
...
function assignUnlabeledEffLandByGlobalNearestSeed(labels){
    const N=W*H;
    let needs=false;
    for(let k=0;k<N;k++){ if(effMask[k]===1 && labels[k]===-1){ needs=true; break; } }
    if(!needs) return 0;
  ...
  function renderLevel(level){
    ...
    const img = vctx.createImageData(W,H); const d=img.data;
    ...
    if(level==='landProvinces'){
        ...
        // ✅ only count seeds that are land
        labelCount = seeds.reduce((n, s) => n + (s.isLand ? 1 : 0), 0);

        legendName = 'Land Provinces';
    }
    else if(level==='provinces'){ mapFn=(p)=>p; pal=palette; labelCount=seeds.length; legendName='All Provinces'; }
    else if(level==='county'){ mapFn=(p)=> provIsLand[p]? provToCounty[p] : -1; pal=countyPalette; labelCount=countyCount; legendName='Counties'; }
    else if(level==='duchy'){ mapFn=(p)=> provIsLand[p]? provToDuchy[p] : -1; pal=duchyPalette; labelCount=duchyCount; legendName='Duchies'; }
    else if(level==='kingdom'){ mapFn=(p)=> provIsLand[p]? provToKingdom[p] : -1; pal=kingdomPalette; labelCount=kingdomCount; legendName='Kingdoms'; }
    else { mapFn=(p)=> provIsLand[p]? provToEmpire[p] : -1; pal=empirePalette; labelCount=empireCount; legendName='Empires'; }
