...
function getDominantCultureLanguageForReligion(r) {
    ...
    // --- Primary source: world.provinces with this religion --------------
    if (Array.isArray(world.provinces)) {
        for (const prov of world.provinces) {
            if (!prov || !prov.culture) continue;

            // religion comparison by object reference
            if (prov.religion === r) {
                bump(prov.culture.language);
            } else if (prov.faith && prov.faith.religion === r) {
                bump(prov.culture.language);
            }
        }
    }

    // --- Fallback: use faith.provinces if any -----------------------------
    if (counts.size === 0 && Array.isArray(r.faiths)) {
        for (const f of r.faiths) {
            if (!f || !Array.isArray(f.provinces)) continue;
    ...
    // --- If still nothing, fall back to existing r.language or random -----
    if (counts.size === 0) {
        if (r.language) return r.language;
        // Last-resort language if absolutely nothing else is available
        if (typeof makeLanguage === 'function') {
            try {
                return makeLanguage(consSet, vowelSet);
            } catch (e) {
                console.warn('getDominantCultureLanguageForReligion: makeLanguage failed, falling back to r.language or "en".', e);
            }
        }
    ...
    let bestLang = null;
    let bestCount = -1;
    for (const [lang, cnt] of counts.entries()) {
        if (cnt > bestCount) {
            bestCount = cnt;
            bestLang = lang;
        }
...
function religionOutputter() {
    // Main religion + faith definitions
    let t = `${daBom}`;
    // Religion + faith localization
    let loc = `${daBom}l_english:\n`;
    // Holy sites file
    let holySites = `${daBom}`;

    if (!world || !Array.isArray(world.religions) || world.religions.length === 0) {
        alert('No religions generated to export.');
        return;
    }
    ...
    for (let i = 0; i < world.religions.length; i++) {
        ...
        t += `  doctrine = doctrine_polytheist\n`
        t += `  doctrine= doctrine_bastardy_legitmization\n`

        for (let n = 0; n < r.doctrines.length; n++) {
            t += `  doctrine = ${r.doctrines[n]}\n`;
        }
        t += `  doctrine = doctrine_no_anointment\n`
        ...
        // Big CK3-style "localization = { ... }" block
        ...
        for (let n = 0; n < r.faiths.length; n++) {
            ...
            // Faith localization into the YAML file
            ...
            // Faith definition into the main religions file
            t += `    ${f.name} = {\n`;
            t += `      color = { ${f.color} }\n`;
            if (f.icon) {
                t += `      icon = ${f.icon}\n`;
            ...
            // Holy sites for this faith
            for (let z = 0; z < f.holySites.length; z++) {
                const pIndex = f.holySites[z];
                const s      = seeds[pIndex];
                if (!s) continue;

                // Keys using same color logic as gtitles
                const baronyKey = baronyKeyFromSeed(s, pIndex);              // b_RxGyBz
                const countyKey = countyKeyFromIndex(s.countyIndex ?? -1);   // c_RxGyBz or fallback
                ...
                t += `      holy_site = ${baronyKey}\n`;

                holySites += `${baronyKey} = {\n`;
                holySites += `  county = ${countyKey}\n`;
                holySites += `  barony = ${baronyKey}\n`;
                holySites += `}\n`;

                loc += `  ${baronyKey}: "${locName}"\n`;
                loc += `  holy_site_${baronyKey}: "${locName}"\n`;
            }
        ...
        // Religion-level localization (gods, afterlife, etc) into YAML
        loc += generateReligionLocalization(r);
    }

    // --- Save the three files like other exporters ------------------------
    if (typeof blobSave === 'function') {
        blobSave(t, 'gen_religions.txt');
        blobSave(loc, 'gen_religions_l_english.yml');
        blobSave(holySites, 'gen_holy_sites.txt');
    } else {
        // Fallback: manual Blob + temporary link (in case blobSave isn't defined here)
        const saveFallback = (text, filename, mime) => {
        ...
        saveFallback(t, 'gen_religions.txt');
        saveFallback(loc, 'gen_religions_l_english.yml', 'text/yaml');
        saveFallback(holySites, 'gen_holy_sites.txt');
    }

    if (typeof setStatus === 'function') {
        setStatus(
            `Exported gen_religions.txt, gen_religions_l_english.yml, gen_holy_sites.txt ` +
...
function generateReligionLocalization(r) {
    ...
    // Use the language of the most prevalent culture following this religion
    const language = getDominantCultureLanguageForReligion(r);
    console.log('Religion', r.name, 'using language', language, 'for localization.');
...
function setReligionLocalization(r) {
    const language = getDominantCultureLanguageForReligion(r);
    console.log('setReligionLocalization for', r.name, 'using language', language);
...
(function addReligionExportButton() {
    ...
    addBtn('downloadGeneratedReligions', 'Export religions + loc + holy sites', () => {
        if (!world || !Array.isArray(world.religions) || !world.religions.length) {
            alert('No religions generated yet. Run the religion generator first.');
            return;
        }
        religionOutputter();
    });
